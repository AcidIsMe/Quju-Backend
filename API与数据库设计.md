...
| status | VARCHAR(30) | NOT NULL, DEFAULT 'pending_activation', CHECK IN ('pending_activation','active','banned') | 账号状态 |
...
| max_participants | INT | NOT NULL, CHECK (>0) | 人数上限 |
...
- FULLTEXT INDEX on (title, description) — 关键词搜索（或用 PostgreSQL GIN + `tsvector`）
...
逻辑：使用 PostgreSQL `ts_query` 对 search_vector 进行全文检索，按相关度排序。
...
3. 校验信誉分 ≥ 活动最低要求
4. 校验名额未满
5. `SELECT * FROM activities WHERE id = :id FOR UPDATE`（行锁）
6. 插入报名记录 + `current_participants + 1`
7. 提交事务
